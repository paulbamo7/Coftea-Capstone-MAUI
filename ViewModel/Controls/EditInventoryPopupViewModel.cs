...
namespace Coftea_Capstone.ViewModel
{
    public partial class EditInventoryPopupViewModel : ObservableObject
    {
        ...
        [RelayCommand]
        private void EditItem(InventoryPageModel item)
        {
            if (item == null) return;
            IsEditInventoryPopupVisible = false;

            // Seed AddItemToInventory with existing values and open in edit mode
            _addItemToInventoryViewModel.IsAddItemToInventoryVisible = true;
            _addItemToInventoryViewModel.ItemName = item.itemName;
            _addItemToInventoryViewModel.ItemCategory = item.itemCategory;
            _addItemToInventoryViewModel.ItemDescription = item.itemDescription;
            _addItemToInventoryViewModel.UoMQuantity = item.itemQuantity;
            _addItemToInventoryViewModel.SelectedUoM = item.unitOfMeasurement;
            _addItemToInventoryViewModel.MinimumQuantity = item.minimumQuantity;
            _addItemToInventoryViewModel.ImagePath = item.ImageSet;
            _addItemToInventoryViewModel.SelectedImageSource = item.ImageSource;
            _addItemToInventoryViewModel.BeginEdit(item.itemID);
        }
